...
template <typename R, typename EventInfoType>
class TransitionWrapper {
 public:
  ...
  template <typename ED, R (*fn)(const ED&)>
  void set(const EventBaseType& event) {
    ::new (&buffer) ED(event);
    fn_v_ptr = &fn_tramp<ED, fn>;
    wrap_type = WrapType::FnWithEvent;
  }

  template <typename ED, R (*fn)(const ED&, void*)>
  void set(const EventBaseType& event, void* ctx) {
    ::new (&buffer) ED(event);
  ...
  template <typename T, R (T::*mem_fn)()>
  void set(T* inst) {
  ...
  template <typename ED, typename T, R (T::*mem_fn)(const ED&)>
  void set(const EventBaseType& event, T* inst) {
    ::new (&buffer) ED(event);
  ...
  void reset() { wrap_type = WrapType::None; }

  operator bool() { return wrap_type != WrapType::None; }
