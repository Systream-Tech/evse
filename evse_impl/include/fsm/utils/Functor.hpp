...
template <typename FunctionType, typename StorageType>
class StaticFunctorWrapper;
...
class StaticFunctorWrapper<R(ArgTypes...), StorageType> {
 public:
  ...
  template <typename T>
  StaticFunctorWrapper& operator=(T lambda) {
    static_assert(sizeof(T) <= sizeof(buf),
                  "Size of passed StorageType is to small for storing the passed functor object");
    // FIXME (aw): we would need to call the destructor by hand, but we restrict usage of this class to cases, where
    // this is not neccessary
  ...
  R operator()(ArgTypes... args) { return (*fn_ptr)(&buf, args...); }

  StorageType buf;

  operator bool() { return fn_ptr != nullptr; }

 private:
  R (*fn_ptr)(void* lambda, ArgTypes...){nullptr};
  template <typename T>
  static R fn_tramp(void* lambda, ArgTypes... args) {
