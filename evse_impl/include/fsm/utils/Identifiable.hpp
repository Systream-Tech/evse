...
struct IdentifiableBase {
  IdentifiableBase(IDType id) : id(id) {}
  const IDType id;
};

template <typename IDType, IDType ID, typename T = void>
struct Identifiable : IdentifiableBase<IDType> {
...
template <typename IDType, IDType ID>
struct Identifiable<IDType, ID, void> : IdentifiableBase<IDType> {
...
template <typename IDType>
struct IdentifiableTypeFactory {
  using Base = IdentifiableBase<IDType>;
  template <IDType ID, typename T = void>
  using Derived = Identifiable<IDType, ID, T>;
};
