...
class FSMInitContext : virtual public FSMCommonContext {
 public:
...
template <typename EventBaseType>
class FSMContext : virtual public FSMCommonContext {
 public:
...
class SyncTransitionWrapper : public TransitionWrapper<R, EventInfoType> {
 public:
  ...
  bool is_internal() { return (this->wrap_type == TransitionWrapper<R, EventInfoType>::WrapType::Internal); }

 private:
  FSMContextType& state_ctx;
};

template <typename EventInfoType, typename StateIDType>
struct StateHandle {
...
class BasicController {
 ...
 public:
  ...
  StateHandleType* current_state() { return cur_state; }
 ...
 private:
  ...
  class FSMContextImpl : public FSMInitContext<EventBaseType>, public FSMContext<EventBaseType> {
   public:
    ...
    void disable_timeout() override final { ctrl.call_state = HandlerCallState::NoCall; }

    void set_event_ptr(const EventBaseType* event_ptr) { this->event = event_ptr; }
