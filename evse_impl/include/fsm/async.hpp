...
class FSMContextCtrl : public FSMContext<EventBaseType> {
 public:
  FSMContextCtrl(ControllerType& ctrl) : ctrl(ctrl){};

  bool submit_event(const EventBaseType& event) override final { return ctrl.submit_event(event); }
...
class BasicController {
 ...
 public:
  enum class InternalEvent { NewTransition, Break };
 ...
 private:
  ...
  enum class InternalState { Idle, NewTransition, Break };
