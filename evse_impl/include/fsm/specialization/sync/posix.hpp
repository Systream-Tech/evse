...
class Repeatable {
 public:
  ...
  void repeat_from_now() { timeout = std::chrono::system_clock::now() + std::chrono::milliseconds(duration); }

  void repeat_from_last() { timeout += std::chrono::milliseconds(duration); }
