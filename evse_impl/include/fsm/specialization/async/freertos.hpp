...
class FSMContextCtrlImpl : public FSMContextCtrl<ControllerType, EventBaseType> {
 public:
  ...
  bool got_cancelled() override final { return cancelled; }
...
class PushPullLock {
 public:
  ...
  PullLock wait_for_pull_lock() { return PullLock(*this); }

  PushLock get_push_lock() { return PushLock(*this); }
...
template <int StackSize = 400, const char* Name = nullptr>
class Thread {
 public:
  Thread() { task_finished = xSemaphoreCreateBinaryStatic(&task_finished_buffer); }
  template <typename T, void (T::*mem_fn)(void)>
  bool spawn(T* class_inst) {
    if (task) {
      return false;
    }
 ...
 private:
  template <typename T, void (T::*mem_fn)(void)>
  static void trampoline(void* pvParameters) {
